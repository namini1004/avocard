...
const sourceStats = [
  ...
  { label: "수집 타깃", value: popularCoverageTargets.length.toString(), sub: `최소 목표 ${minimumPopularCardCoverage}개` },
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
        <section className="rounded-[2.5rem] bg-ink p-8 text-white shadow-lift md:p-10">
          ...
          <h1 className="mt-3 max-w-4xl text-5xl font-black leading-tight">공식 출처 기반 100개 카드 수집 현황</h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-white/72">
            여기는 실제 분석 완료 카드 목록이 아니라, 카드사 공식 자료로 채워야 할 수집 타깃 현황입니다. 검수가 끝난 카드는 카드 목록에 반영됩니다.
          </p>
        </section>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {officialIssuerSources.map((issuer) => (
                <div key={issuer.code} className="rounded-3xl border border-avocado-900/10 p-5">
                  ...
                  <Link href={issuer.productListUrls[0]} className="mt-4 inline-flex text-sm font-black text-avocado-700">
                    공식 페이지 열기
                  </Link>
                </div>
              ))}
        ...
        <section className="mt-8 rounded-[2rem] bg-white p-6 shadow-soft">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <h2 className="text-2xl font-black text-ink">100개 수집 타깃</h2>
              <p className="mt-2 max-w-4xl text-sm leading-6 text-ink/58">
                각 타깃은 실제 카드 1개 이상을 공식 출처로 매핑하기 위한 작업 큐입니다. 카드별 원문과 PDF가 붙으면 `draft → needs_review → verified` 순서로 공개됩니다.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {popularCoverageTargets.map((target) => (
              <div key={target.id} className="rounded-3xl border border-avocado-900/10 bg-cream p-4">
                ...
                <p className="mt-3 text-xs leading-5 text-ink/48">{target.candidateQuery}</p>
              </div>
            ))}
          </div>
        </section>
